...
- Telling which language a selected phrase is in, before it is translated, uses the browser's built-in detector (`i18n.detectLanguage`: CLD2 compiled into Firefox, CLD3 compiled into Chromium) - on the device, with no model download and no request; the text never leaves the browser. Safari has no such detector, and re/read then skips the check.
