...
- **Appearance.** Light, sepia and dark themes, serif or sans type - or any font installed on the device, typed by name in the settings and offered as **Custom** in the reader's **Type** row - text size, column width, line spacing, left-aligned or justified lines, hyphenation, and paragraphs set apart by a blank line or by an indented first line, as in a book; links can be shown as plain text, so a book is shown without blue links. Hyphenation is the browser's own, in the language the page or book declares: which languages it covers depends on the browser and the system, a page that declares no language is not hyphenated, and nothing is downloaded for it (see [PRIVACY.md](PRIVACY.md), "Hyphenation"). A small tab at the edge of the reader's bar hides the bar; the tab stays at the window's edge, and pressing it again brings the bar back.
