...
- **Look up a word without a page.** A field in the toolbar popup, and **Add a phrase** on the saved phrases page, take a word or phrase you type: the bubble's dictionary entries appear under it, a press on a meaning saves the phrase with that meaning, and **Own meaning** lets you write one yourself. The translation model is not asked here on purpose - a word on its own has no sentence around it, which is where the model guesses worst.
