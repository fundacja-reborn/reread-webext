...
- **The sentence with the phrase.** One setting, **Save the sentence with the phrase** (off until you turn it on), keeps with every phrase you save from the bubble the sentence it stood in - the sentence as the page shows it, never its translation. It is shown under the phrase on the saved phrases page, folded to one line, and it is what makes a sentence card in Anki (below). A phrase keeps the first sentence it was saved from; saving it again changes the meanings and leaves the sentence alone. A phrase saved without a sentence - before you turned the setting on, from the **Add a phrase** field, or from a two-column file - takes the sentence it stands in the next time you open its bubble on a page. The bubble over a saved phrase does not show it - while you read, the sentence in front of you is the one that matters.
...
- **Backup.** **Export** on the reading list writes one `reread-backup.zip` with everything re/read keeps: the reading list with its highlights and reading positions (and its pictures when you tick the box), every saved phrase of every language pair with its sentence and counts, every document's highlights - books' too - and the settings. **Import** reads that file back part by part, adding what is missing and never overwriting what is here: an article already saved keeps its copy, a phrase keeps its meanings and takes only a sentence it lacks or a higher count, a highlight already here is left out, and the settings are restored only when you tick the box. The older files - the list's `.json` or `.zip`, the highlights' `.json` - still import. Press **Select**, tick some articles and export only those to get the list's own file for sharing. After every export a line under the button says what went into which file, and its size. Books are not in the backup - a book's backup is its `.epub`; its highlights come back from the backup once the same book is imported again. Vocabulary alone travels as TSV from the phrases page.
