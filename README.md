...
- **Offline reading list.** A saved article is stored in full on your device: it opens with no network, also when the original page has moved or disappeared. Pages opened in the reader are saved by default - one setting turns that off, and a page already in the list is never overwritten. Pictures are not saved by default: **Download pictures** in the reader's menu stores an article's pictures with it, on one press, and the list shows how many pictures an article keeps and how much space they take. Every row also shows how long the text is - its words, and about how many minutes they take at your own reading speed, which you type once in the settings (200 words a minute until you do) - so you can pick what to read next by its length; the same two numbers appear under an article's title in the reader, together with how many of your saved phrases were found in that text.
- **EPUB books.** Import a book into the reading list; long books are split into parts, and a table of contents is built from the chapter headings. A footnote opens in a small card next to its number. The pictures in the book file are kept with it and shown in the text (scaled down to screen size where that saves space); the list shows how many a book keeps and how much space they take, and **Remove pictures** in the reader's menu deletes them. A book's row shows how many words the whole book holds and about how many hours it takes; the line under a part's title shows the same for that part.
- **Reading position.** Every saved document reopens where you stopped.
- **Export a document.** Any saved article or imported book can leave as a file of its own: **Export as EPUB** in the reader's menu writes a book file for e-readers and reading apps - the text as the reader shows it, the pictures kept with the document, the headings as its table of contents, a book's footnotes as pop-up notes - and **Export as Markdown (.md)** writes a plain-text page with headings, lists, quotes and links (an article's pictures by their web addresses) for notes and other apps. The same two buttons stand inside the reading list's selection, one file per ticked document. The file is named after the title and written from the stored copy on this device; nothing is fetched for it.
...
- **Appearance.** Light, sepia and dark themes, serif or sans type - or any font installed on the device, typed by name in the settings and offered as **Custom** in the reader's **Type** row - text size, column width, line spacing, left-aligned or justified lines, hyphenation, and paragraphs set apart by a blank line, by an indented first line as in a book, or by both; links can be shown as plain text, so a book is shown without blue links. Hyphenation is the browser's own, in the language the page or book declares: which languages it covers depends on the browser and the system, a page that declares no language is not hyphenated, and nothing is downloaded for it (see [PRIVACY.md](PRIVACY.md), "Hyphenation"). A small tab at the edge of the reader's bar hides the bar; the tab stays at the window's edge, and pressing it again brings the bar back.
