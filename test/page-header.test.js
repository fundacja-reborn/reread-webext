...
describe("the bar stuck to the top of every page", () => {
  ...
  it("says a tool in hand by a real wash alone, so that 16 greys keep it and the bar stays quiet, on every page's bar", async () => {
    ...
    // The wash is the root's token since D237, so a palette can say
    // otherwise: the lit rule reads nothing else, and draws nothing else.
    assert.match(lit, /background: var\(--page-pressed-bg\);/, "the lit tool lights itself by a wash of its own");
    assert.doesNotMatch(lit, /box-shadow/, "the lit tool wears a ring inside its frame again (D241)");
    ...
    // 2026-09-14), and D237's e-ink ring was too loud for the Aa panel in
    // its turn (D241) - the wash is the one signal on every paper, inside
    // the frame the control already wears.
    assert.doesNotMatch(styles, /--page-pressed-ring/, "a paper lights its controls with a ring again");
  ...
  it("is the one stuck strip over a list: the selection's bar scrolls with the rows, under it", async () => {
    ...
    // The ninth is the black strip a room draws under a phone's cutout in
    // full screen (D269) - the bar's own strip stands at 2 and a room at
    // 4, so a room has to draw its own or the lens sits over white again.
    assert.equal((styles.match(/position: (?:sticky|fixed)/g) ?? []).length, 9, "a strip of chrome beyond the speech bar, the two curtains, the page count, the edge line, the room and its cutout strip, the panel's foot and the turn's flash is stuck or fixed on the reader page");
